...
All in-game commands are prefixed with "//sbx" or "//scoreboardx", for
example: "//sbx help".
...
* REPORT [<target>]
  ...
  //sbx report          Reports to current chatmode
  //sbx report l        Reports to your linkshell
  //sbx report t suji   Reports in tell to Suji
...
* REPORTSTAT <stat> [<playerName>] [<target>]
  ...
  Examples:
  //sbx reportstat acc       -- Sends acc report your default chatmode
  //sbx rs crit              -- Same as above
  //sbx rs crit p            -- Explicitly to party
  //sbx rs acc tell suji     -- Sends acc to Suji
  //sbx rs acc t suji        -- Same as above
  //sbx rs acc tulia t suji  -- Report accuracy for Tulia only and send it in tell to Suji
...
* STAT <statname> [<player>]
  ...
  Examples:
  //sbx stat acc            Shows accuracy for everyone
  //sbx stat crit Flippant  Only show crit rate for Flippant
...
* sbxcolor - Color of scoreboard's chat log output
* showallidps - Set to true to display the alliance DPS, false otherwise.
* resetfilters - Set to true if you want filters reset when you "//sbx reset", false otherwise.
